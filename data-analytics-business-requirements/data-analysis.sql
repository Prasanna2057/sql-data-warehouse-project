...
SELECT
    trunc(order_date, 'YYYY')    AS order_date,
...
-- and the running total of sales and moving average price over time (Cumulative Analysis)
 SELECT
    order_date,
    total_sales,
    SUM(total_sales)
...
SELECT
    ...
    round(AVG(current_sales)
          ...
          2) AS avg_sales,
    current_sales - round(AVG(current_sales)
                          OVER(PARTITION BY product_name),
                          2)     AS diff_avg,
    CASE
        WHEN current_sales - round(AVG(current_sales)
                                   OVER(PARTITION BY product_name),
                                   2) > 0 THEN
            'Above Average'
        WHEN current_sales - round(AVG(current_sales)
...
WITH product_segments AS (
    SELECT
        ...
        CASE
            WHEN cost < 100                THEN
                'Below 100'
            WHEN cost BETWEEN 100 AND 500  THEN
                '100-500'
            WHEN cost BETWEEN 500 AND 1000 THEN
                '500-1000'
            ELSE
                'Above 1000'
        END cost_range
